(function($){
	...
	$.extend($.fn.datagrid.methods, {
		autoSizeColumn: function(jq, field){
			return jq.each(function(){
				var fc = $(this).datagrid('getpanell').find('.datagrid-header .datagrid-filter-c');
				// fc.hide();
				fc.css({
					width:'1px',
					height:0
	...
	function resizeFilter(target, field){
		...
		var dg = $(target);
		var header = dg.datagrid('getpanell').find('div.datagrid-header');
	...
	function getFilterComponent(target, field){
		var header = $(target).datagrid('getpanell').find('div.datagrid-header');
		...
		destroyFilter: function(jq, field){
			return jq.each(function(){
				...
				if (field){
					...
					$(this).datagrid('getpanell').find('.datagrid-header .datagrid-filter-row').remove();
